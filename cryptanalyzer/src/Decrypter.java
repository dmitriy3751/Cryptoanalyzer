 ...
 * класс расшифрования текста, используется шифр Цезаря
...
public class Decrypter {
    ...
    public Decrypter(String inputFileName, int key, String alphabet) {
    ...
    public void makeDecryption(){
        // метод, выполняющий расшифрование

        try (BufferedReader br = new BufferedReader (new FileReader(this.inputFileName));
             BufferedWriter bw = new BufferedWriter(new FileWriter("decoded text.txt")))
        {
            // чтение посимвольно
            int symbol;
            while((symbol=br.read())!=-1){
                int encryptedSymbol = this.decryptSymbol(symbol);
    ...
    private int decryptSymbol(int symbol){
